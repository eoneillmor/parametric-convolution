...
{   
    ...
    for (int i = 0; i < 3; i++) 
    {
        ...
        while (!fileLocation.eof())
        {
            ...
            if (i == 1)
            {
                potlidAmpES[filterNum] = ::atof(value.c_str()) * 2.5;
            }
            if (i == 2)
            {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        while (!fileLocation.eof())
        {
            ...
            if (i == 1)
            {
                BanjoAmpES[filterNum] = ::atof(value.c_str()) * 5;
            }
            if (i == 2)
            {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        while (!fileLocation.eof())
        {
            ...
            if (i == 1)
            {
                BazoukiAmpES[filterNum] = ::atof(value.c_str())*3;
            }
            if (i == 2)
            {
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        while (!fileLocation.eof())
        {
            ...
            if (i == 1)
            {
                GuitarAmpES[filterNum] = ::atof(value.c_str())*0.4;
            }
            if (i == 2)
            {
    ...
    addParameter(mAlphParam = new juce::AudioParameterFloat("alph", "Alph", 0.000, 2.0, 1.0));
...
void FilterTestAudioProcessor::calculate_coefficients(float f, float alpha, float A, int IR)
{
    ...
    for (int i = 1; i < M; i++) {
        ...
        b1[i] = (A_i * R) * (C * sin(omegaT) - S * cos(omegaT));
        b0[i] = A_i * S;
...
void FilterTestAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    calculate_coefficients(mFreqParamSmooth, mAlphParamSmooth, 0.25, mIR);
